class PostsController < ApplicationController
  ...
  def create
    ...
    if the_post.valid?
      the_post.save
      redirect_to("/board/#{the_post.board_id}", { :notice => "Post created successfully." })
    else
      redirect_to("/board/#{the_post.board_id}", { :alert => the_post.errors.full_messages.to_sentence })
    end
  end
